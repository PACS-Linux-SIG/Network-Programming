/* echoClientUDP.c - connects to a server, send a message, waits for a */
...
int main()
{
  ...
  int serverAddressSize = 0; /*place to store size of serverAddress structure*/
  ...
  printf( "%s", MSG );
  if ( sendto( serverSocket, MSG, strlen( MSG )+1, 0, (struct sockaddr *)&serverAddress, sizeof( serverAddress ) ) < 0 )
  {
  ...
  if ( recvfrom( serverSocket, data, DATA_SIZE, 0, (struct sockaddr *)&serverAddress, &serverAddressSize ) < 0 )
  {
